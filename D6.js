...
let numbersArray = [1, 4, 5, 6, 9];

function sumNumbers(arr) {
  const sum = arr.reduce((accumulatore, valoreCorrente) => {
    console.log("ACC", accumulatore);
    console.log("VAL", valoreCorrente);
    return accumulatore + valoreCorrente;
  }, 0);
}
let sum1 = sumNumbers(numbersArray);
console.log(sum1);
...
function incrementoArray(arr, n) {
  return arr.map((num) => num + n);
}

console.log(incrementoArray([1, 2, 3, 4, 5], 2));
...
function getMoviesFromMillennium(movies) {
  return movies.filter((movie) => {
    return movie.Year >= 2000;
  });
}

console.log(getMoviesFromMillennium(movies));
...
console.log(getMovieByImdbID("tt0087365"));

/* ESERCIZIO 15 (findIndex)
  Scrivi una funzione per ottenere dall'array fornito l'indice del primo film uscito nell'anno fornito come parametro.
*/

function findFirstMovieIndex(year) {
  return movies.findIndex((movie) => movie.Year === year);
}

console.log(findFirstMovieIndex("1963"));
console.log(findFirstMovieIndex("2012"));
